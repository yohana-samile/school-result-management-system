    ...
    <div id="wrapper">
        ...
        <ul class="navbar-nav sidebar accordion" id="accordionSidebar" style="background-color:  rgb(235, 233, 233">
            ...
            <a class="sidebar-brand d-flex align-items-center justify-content-center" href="{{url('home')}}">
                ...
                <div class="sidebar-brand-text mx-3 text-dark">{{__("SRMS")}}</div>
            </a>
            <!-- Divider -->
            ...
            <li class="nav-item">
                ...
                <div id="collapseResult" class="collapse" aria-labelledby="headingResult" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        <h6 class="collapse-header">{{__("Student Result:")}}</h6>
                        @if ($firstRole->name != 'is_student')
                            @if ($semesters->isNotEmpty())
                                @foreach ($semesters as $value)
                                    <a class="collapse-item" href="{{ url('result/index', ['id' => $value->id ])}}">{{ $value->name }}</a>
                                @endforeach
                            @endif
                        @else
                            <a class="collapse-item" href="{{ url('result/my_result')}}">{{__('My Result')}}</a>
                        @endif
                    </div>
                </div>
            </li>
